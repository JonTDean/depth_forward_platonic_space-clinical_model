...
The `dfps_configuration` crate loads namespace-aware dotenv files automatically. Every crate opts into a namespace (e.g., `app.web.api`), and the loader reads `.env.<namespace>.<profile>` from the workspace root based on `DFPS_ENV` (default: `dev`). You can override the filename entirely with `DFPS_ENV_FILE`.
...
For quickstarts, set `DFPS_ENV=dev` (default) and create the companion `.env` files. Override values only if ports conflict with other services.

### 3.1 Manage `.env` files

Keep per-service environment files in the repo root so every crate sees the same layout. Example development files:
